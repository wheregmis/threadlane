...
impl ProjectRegistry {
    pub fn load(global_mypi_dir: &Path) -> Result<Self, ProjectRegistryError> {
        let registry_path = registry_path(global_mypi_dir);
...
fn registry_path(global_mypi_dir: &Path) -> PathBuf {
    global_mypi_dir.join(REGISTRY_DIR).join(REGISTRY_FILE)
}
    ...
    impl TempDir {
        fn new(test_name: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "mypi-project-registry-{test_name}-{}-{id}",
                std::process::id()
            ));
