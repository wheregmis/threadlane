...
) -> Vec<threadlane_acp::AcpAgentRecord> {
    threadlane_acp::AcpManager::new(default_global_threadlane_dir(), project_root)
        .configs()
        .into_iter()
        .map(|config| threadlane_acp::AcpAgentRecord {
            status: if config.enabled {
                threadlane_acp::AcpAgentStatus::Connecting
            } else {
                threadlane_acp::AcpAgentStatus::Disconnected
            },
            config,
        })
        .collect()
