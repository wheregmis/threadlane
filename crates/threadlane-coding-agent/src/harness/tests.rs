...
fn provider_boundary_retains_and_budgets_current_system_after_reload() {
    ...
    for overflow_recovery in [false, true] {
        ...
        let actual =
            estimate_request_tokens(&prepared.messages, None, &CompactionParams::from(&config));
...
fn adaptive_compaction_commits_before_next_provider_attempt() {
    ...
    let start_seq = harness
        ...
        .iter()
        .find_map(|record| match record {
            HarnessRecord::ProviderRequestStarted { seq, .. } if *seq > compacted_seq => Some(*seq),
            _ => None,
        })
...
fn manual_compaction_telemetry_reports_true_pre_post_and_removed_count() {
    ...
    let pre_tokens = estimate_request_tokens(&before, None, &CompactionParams::from(&config));
    ...
    let retained_tail_tokens = estimate_request_tokens(
...
fn cancellation_closes_an_unfinished_provider_attempt_before_abort_observation() {
    ...
    let abort_observed_seq = store.records().iter().find_map(|record| match record {
        HarnessRecord::AbortObserved {
            seq, wall_time_ms, ..
        } => {
            assert!(
                wall_time_ms.is_some(),
                "abort time must survive journal reload"
            );
            Some(*seq)
        }
        _ => None,
...
fn subagent_start_returns_accepted_child_run_while_main_is_busy() {
    ...
    assert_eq!(
        harness
            ...
            .iter()
            .filter(
                |entry| entry.lane == "main" && matches!(entry.message, AgentMessage::User { .. })
            )
            .count(),
        1
    );
    ...
    harness
        .append_message(AgentMessage::Assistant {
            ...
            tool_calls: Some(vec![threadlane_provider::openai::ToolCall {
                ...
                function: threadlane_provider::openai::ToolCallFunction {
                    name: "read_file".into(),
                    arguments: r#"{\"path\":\"README.md\",\"start_line\":1,\"end_line\":1}"#.into(),
                },
                thought_signature: None,
            }]),
            stop_reason: None,
...
async fn context_snapshot_capture_persists_fuzzy_read_actual_path() {
    ...
    assert_eq!(
        super::super::context_snapshots::resolve_context_snapshot(&path, dir.path(), &context_id,)
            .unwrap()
            .content,
        read_output
    );
...
async fn compacted_context_snapshot_stays_durable_and_is_indexed_in_checkpoint() {
    ...
    let config = AgentConfig::default();
    let before = harness.model_context("main").unwrap().messages();
    let prepared = compact_for_budget(&before, None, 1, &CompactionParams::from(&config)).unwrap();
    harness
        .commit_prepared_compaction(
            &run_id,
            "unknown/test-model",
    ...
    let provider_checkpoint =
        threadlane_provider::convert_to_llm(std::slice::from_ref(checkpoint_message))[0]["content"]
            .as_str()
            .unwrap()
            .to_owned();
    ...
    let post_tokens =
        estimate_request_tokens(&compacted_messages, None, &CompactionParams::from(&config));
    assert_eq!(
        harness
            .store
            .records()
    ...
    let before = harness.model_context("main").unwrap().messages();
    let prepared =
        compact_for_budget(&before, None, 1, &CompactionParams::from(&second_config)).unwrap();
    harness
        .commit_prepared_compaction(
            &run_id,
            "unknown/test-model",
...
#[tokio::test]
async fn tool_intent_and_completion_survive_harness_reload() {
    let (_dir, path) = temp_session();
    let mut harness = CodingSessionHarness::open(&path).unwrap();
    harness
        .begin_run("run-reload", AgentMessage::user("prompt", vec![]))
        .unwrap();
    harness
        .append_message(AgentMessage::Assistant {
            content: None,
    ...
    harness
        .record_tool_result(
            "run-reload",
            &AgentToolResult::external("call-reload", "read_file", "body", false),
        )
        .unwrap();

    let reopened = CodingSessionHarness::open(&path).unwrap();
    let state = Reducer::reduce(&reopened.store).unwrap();
    let tool = &state.lane("main").unwrap().tools[0];
    assert_eq!(tool.tool_call_id, "call-reload");
    assert_eq!(tool.tool_name, "read_file");
    assert!(tool.completed);
}

#[tokio::test]
async fn durable_event_subscription_replays_harness_commits() {
    let (_dir, path) = temp_session();
    let mut harness = CodingSessionHarness::open(&path).unwrap();
    let reopened = CodingSessionHarness::open(&path).unwrap();
    let mut replay = reopened.subscribe_durable_events().unwrap();
