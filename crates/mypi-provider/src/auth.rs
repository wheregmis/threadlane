...
pub fn save_credentials(tokens: &OAuthTokens) -> Result<(), String> {
    ...
    let creds = StoredCredentials {
        ...
        source: "~/.mypi/credentials.json".to_string(),
    };
...
pub fn load_credentials() -> Option<StoredCredentials> {
    ...
    // 1. Try ~/.mypi/credentials.json
    let mypi_path = get_credentials_path();
    if mypi_path.exists() {
        if let Ok(content) = fs::read_to_string(&mypi_path) {
...
mod tests {
    ...
    fn test_credentials_path() {
        let path = get_credentials_path();
        assert!(path.to_string_lossy().contains(".mypi"));
    }
}
