//! Context-window budgeting for Threadlane: model limits and token budgets.
//!
//! `UNKNOWN_MODEL_CONTEXT_LIMIT`, the hardcoded model table,
//! `BudgetConfig`, `ContextBudget`, and `context_budget` live here so the
//! engine, session, and UI crates share one definition without depending on
//! the runtime. Model limits resolve through the provider registry first
//! (`threadlane_provider::model_registry::context_window_for`) with the
//! hardcoded table as fallback. `threadlane-runtime` keeps a thin adapter
//! (`model_metadata` shim) translating its `AgentConfig` into
//! `BudgetConfig`; new code should import `threadlane_context` directly.

/// Fallback context limit for models with no known window.
pub const UNKNOWN_MODEL_CONTEXT_LIMIT: usize = 128_000;

/// Tuning knobs for budget math. Mirrors the `context_*` fields of the
/// runtime `AgentConfig`; the runtime adapter converts between them (pinned
/// by a default-drift test) so this crate stays engine-free.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BudgetConfig {
    pub unknown_model_context_limit: usize,
    pub context_minimum_headroom_tokens: usize,
    pub context_headroom_percent: usize,
    pub context_repeated_input_ceiling_tokens: usize,
    pub context_minimum_retained_tail_tokens: usize,
    pub context_maximum_retained_tail_tokens: usize,
    pub context_retained_tail_percent: usize,
}
...
impl ContextBudget {
    fn from_limit(limit: Option<usize>, config: &BudgetConfig) -> Self {
