...
//!
//! This crate acts as an ACP *client*: it launches an external agent as a
...
//!
//! This crate owns the protocol and configuration layers only. It deliberately
//! contains no UI wiring and no host-application integration: callers supply an
//! [`AcpClientHandler`] and drive [`AcpSession`] or [`AcpConnection`] directly.
//! Filesystem access in [`AcpWorkspaceClient`] is workspace-scoped through an
//! injectable path validator (see [`AcpWorkspaceClient::with_path_validator`]);
//! the default validator canonicalizes the nearest existing ancestor so
//! symlinked workspace spellings (e.g. `/tmp` vs `/private/tmp` on macOS)
//! compare correctly. Host applications with their own workspace guard should
//! inject it to keep a single path-policy implementation.
//!
//! Configuration uses `acp.json` in a global directory and in a project
//! directory, with project entries shadowing global entries that share an id.
...
/// Validates a requested agent path against a workspace root.
///
/// Takes the requested path string and the canonical workspace root, and
/// returns the validated absolute path or a human-readable rejection.
/// Host applications should inject their own workspace guard via
...
/// resolves a not-yet-existing target by canonicalizing its nearest existing
/// ancestor, and rejects anything outside the canonical root.
///
...
pub fn default_path_validator(requested: &str, workspace_root: &Path) -> Result<PathBuf, String> {
    ...
    // Walk up to the nearest existing ancestor, canonicalize it, then re-join
    // the remaining components so not-yet-existing targets still resolve.
    ...
    let canonical_ancestor = ancestor
        .canonicalize()
        .map_err(|e| format!("Failed to resolve '{requested}': {e}"))?;
...
pub struct AcpWorkspaceClient {
    ...
    path_validator: AcpPathValidator,
}
...
impl AcpWorkspaceClient {
    pub fn new(workspace_root: PathBuf) -> Self {
        Self {
            ...
            updates: None,
            path_validator: Arc::new(default_path_validator),
        }
    }
    ...
    fn resolve(&self, path: &str) -> Result<PathBuf, String> {
        (self.path_validator)(path, &self.workspace_root)
    }
}
