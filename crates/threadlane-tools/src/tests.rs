...
fn list_dir_returns_typed_error_for_regular_file() {
    ...
    let result = try_execute_tool_in_workspace("list_dir", r#"{"path":"file.txt"}"#, dir.path());
...
fn list_dir_preserves_successful_output() {
    ...
    let typed = try_execute_tool_in_workspace("list_dir", r#"{"path":"."}"#, dir.path()).unwrap();
...
fn edit_files_hashline_commits_all_files() {
    ...
    try_execute_tool_in_workspace("edit_files_hashline", &args.to_string(), dir.path()).unwrap();
    assert_eq!(
        std::fs::read_to_string(dir.path().join("a.txt")).unwrap(),
        "first\n"
    );
...
fn test_edit_file_hashline_schema_description() {
    ...
    let start_anchor_desc = params["edits"]["items"]["properties"]["start_anchor"]["description"]
        .as_str()
        .unwrap();
...
fn test_workspace_containment_command_cwd_escape() {
    let dir = tempdir().unwrap();
    let root = dir.path();
    let res = execute_tool_in_workspace("run_command", r#"{"command": "ls", "cwd": "/tmp"}"#, root);
    assert!(res.contains("Access denied"));
}
...
fn test_consolidate_memory_preserves_unmanaged_content() {
    ...
    let merged = consolidate_memory_entries(existing, &["New architecture".to_string()], &[], &[]);
...
fn test_read_file_virtual_schemes_and_urls() {
    ...
    let mr_url_payload =
        json!({ "path": "https://gitlab.com/gitlab-org/gitlab/-/merge_requests/99" }).to_string();
...
fn typed_execution_keeps_successful_calls_successful() {
    ...
    let read =
        try_execute_tool_in_workspace("read_file", r#"{"path":"sample.txt"}"#, dir.path()).unwrap();
...
fn test_read_file_auto_resolves_unique_workspace_suffix() {
    ...
    assert!(
        result.contains("[Notice: Auto-resolved 'src/view.rs' to 'crates/my-crate/src/view.rs']")
    );
    ...
    assert!(
        result_bare.contains("[Notice: Auto-resolved 'view.rs' to 'crates/my-crate/src/view.rs']")
    );
...
fn test_edit_file_hashline_auto_resolves_unique_workspace_suffix() {
    ...
    assert!(
        read_res.contains("[Notice: Auto-resolved 'src/view.rs' to 'crates/my-crate/src/view.rs']")
    );
    ...
    assert!(
        edit_res.contains("[Notice: Auto-resolved 'src/view.rs' to 'crates/my-crate/src/view.rs']")
    );
