...
pub(crate) struct QuestionManager {
...
impl QuestionManager {
    ...
    pub(crate) fn handle(&self) -> QuestionHandle {
        self.handle.clone()
    }
}
...
pub(crate) struct AskQuestionToolExecutor {
    handle: QuestionHandle,
    event_tx: broadcast::Sender<AgentEvent>,
}

impl AskQuestionToolExecutor {
    pub(crate) fn new(handle: QuestionHandle, event_tx: broadcast::Sender<AgentEvent>) -> Self {
    ...
    async fn interactive_ask_resolves_through_the_handle() {
        ...
        let resolved = handle.resolve(
            ...
            QuestionAnswer {
                ...
                answers: vec![threadlane_runtime::QuestionItemAnswer {
                    question_id: "q1".into(),
                    selected: vec!["a".into()],
                    custom_text: None,
                }],
