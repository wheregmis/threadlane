...
pub fn load_prompt_templates(project_dir: &Path, global_dir: &Path) -> Vec<PromptTemplate> {
    ...
    // 1. Global prompts: ~/.mypi/prompts/
    let global_prompts = global_dir.join("prompts");
    templates.extend(load_prompt_templates_from_dir(&global_prompts, "global"));

    // 2. Project prompts: <project>/.mypi/prompts/
    let project_prompts = project_dir.join(".mypi/prompts");
    ...
    // 3. Package prompts: <project>/.mypi/packages/*/prompts/
    let project_packages_prompts = project_dir.join(".mypi/packages");
