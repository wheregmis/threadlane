...
- The model can ask structured clarifying questions through the `ask_question` tool (`threadlane-session/src/question.rs`: `QuestionManager`/`QuestionHandle`, `AgentEvent::QuestionRequested`). Unattended sessions fail the call fast instead of blocking; GPUI has no answer UI yet, so it posts a visible transcript notice and dismisses the request immediately — never leave a question pending with no resolver.
