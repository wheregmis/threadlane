...
- The computer-use mirror (`screens/computer_mirror`, a non-activating `WindowKind::PopUp`) polls `<work_dir>/.threadlane/previews/latest.json`, which the session tools rewrite on every screenshot/act — polling keeps the session crate UI-free. Captures composite explicit window IDs minus our PID (`create_image_from_array`), so the always-on-top popup cannot recurse; blank composites fall back to `screencapture`. The chat stream pump opens the popup once per unseen computer trigger (`AppState::take_computer_mirror_trigger`, guarded by `mirror_open`).
- Live frames come from an in-process 2fps `CGWindowList` poller (`computer_stream.rs`), not `ScreenCaptureKit`: its Rust bindings link Swift runtime dylibs that break the build and packaging. `CGWindowListCreateImageFromArray` wants raw `u32` window numbers (`CFArray::from_copyable`), not `CFNumber` objects — the object form returns NULL. Serve stream frames only on explicit screenshot calls, never auto-attached; the poller thread exits after two idle minutes.
