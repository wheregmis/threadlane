...
- The GPUI crate holds no local session-projection or JSONL-parsing code; every projection (hydration, trajectory, metrics, token usage, context window) is computed by the daemon and delivered over `threadlane-protocol` RPC. Do not reintroduce local `JsonlStore`-based projections or `legacy-api-tests`-style gated fallbacks in `threadlane-gpui`.
...
- GPUI never derives session transcript paths from a presumed on-disk layout. Every transcript path comes from daemon-returned data (`SessionInfo.session_file`, or `SessionSummary.session_file` returned by `create_session`, which the daemon must keep populating); `AppState::session_file` resolves only from daemon-known sources and returns `None` otherwise. New sessions are created synchronously before the first prompt so the daemon-returned path is available immediately.
...
- Provider authentication and Codex account state are daemon-owned. GPUI must list, activate, and remove accounts through daemon RPC rather than direct credential-file access or compatibility stubs. A successful browser/device login activates the account it just authenticated, and daemon session credential resolution must use `CodexAccountsStore::active_account()` rather than the first stored account, or an invalidated older token can remain in use.
- Antigravity uses Google Cloud Code Assist's `v1internal` endpoints and outer request envelope, not the public Gemini `streamGenerateContent` endpoint. Preserve project discovery, production/daily endpoint fallback, runtime-model mapping, wrapped SSE parsing, and provider-specific tool schemas when changing that client. If every endpoint fails, preserve a production `RESOURCE_EXHAUSTED`/HTTP 429 as the final actionable error instead of masking it with a later sandbox 404.
